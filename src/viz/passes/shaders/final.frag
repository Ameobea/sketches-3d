...
void main() {
  ...
  // Coverage of what composited over the scene after the opaque render (volumetric fog,
  // particles), exported in its alpha. Attenuates the distance fog so it doesn't repaint them
  // using the depth of geometry beneath. ParticlePass applies the same coverage to emissiveRT.
...
#ifdef HAS_EMISSIVE_BUFFER
{
  ...
  // Additive emissive particles can push HalfFloat alpha past 1.
  emissive.a = min(emissive.a, 1.0);
