...
const PERLIN_PERM_TABLE: [u8; 256] = [
  ...
  178, 140, 136, 172, 247, 22, 155, 34, 154, 243, 224, 105, 253, 78, 94, 162, 193, 160, 187, 55, 3,
  49, 233, 86, 114, 5, 227, 36, 183, 118, 159, 230, 200, 61, 46, 38, 143, 7, 217, 83, 119, 84, 28,
  23, 104, 79, 8, 99, 66, 69, 239, 64, 133, 59, 58, 153, 17, 124, 240, 170, 40, 108, 107, 219, 147,
  185, 188, 52, 33, 158, 196, 176, 163, 151, 111, 135, 92, 10, 177, 169, 21, 228, 117, 4, 175, 209,
  198, 20, 120, 1, 43, 220, 106, 54, 186, 244, 44, 63, 130, 131, 199, 67, 110, 71, 123, 189, 234,
  157, 222, 45, 194, 128, 95, 252, 212, 204, 60, 152, 82, 116, 202, 156, 14, 50, 190, 145, 179,
  203, 139, 164, 77, 91, 221, 208, 171, 181, 137, 72, 57, 211, 97, 13, 126, 141, 132, 100, 51, 250,
  168, 90, 15,
];
...
pub fn fbm_2d_tileable(
  ...
  for octave_ix in 0..octaves {
    ...
    value += periodic_perlin_noise_2d(
      seed + octave_ix as u32,
      cells as isize,
      pos * (cells / period),
    ) * amp;
    freq *= lacunarity;
...
"#,
    ...
    assert_eq!(
      getf("plain").to_bits(),
      fbm_2d(0, 4, 1., 0.5, 2., Vec2::new(0.4, 0.7)).to_bits()
    );
