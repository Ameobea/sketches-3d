...
use crate::{
  analysis::Analysis, AnalysisCtx, AnalysisDiagnostic, AnalysisResult, DiagnosticSeverity,
};
...
fn exit_positions(ctx: &AnalysisCtx, program: &Program, diagnostics: &mut Vec<AnalysisDiagnostic>) {
  for (loc, message) in check_exit_positions(program) {
    let (line, col) = ctx.eval_ctx.resolve_loc(loc);
    if (line, col) == (0, 0) {
      continue;
    }
    let width = if message.starts_with("`return`") {
      6
    } else {
      5
    };
