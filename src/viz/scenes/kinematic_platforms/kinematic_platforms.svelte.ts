...
const initLevel = async (viz: VizState, matsPromise: ReturnType<typeof buildMaterials>) => {
  ...
  const { checkpointMat, greenMosaic2Material, goldMaterial, pylonMaterial } = await matsPromise;
  ...
  const kinematicCubeMat = new THREE.MeshStandardMaterial({ color: 0xff0033 });
  ...
  viz.registerBeforeRenderCb(curTimeSeconds => {
    ...
    kinematicCube.position.set(-15 + Math.sin(t) * 10, -4, 0);
    // kinematicCube.rotation.set(0, 0, 0);
    kinematicCube.rotation.set(0, t, 0);
