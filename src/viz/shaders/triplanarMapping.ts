...
type SampleExprBuilder = (sampler: string, uv: string, mean: string) => string;
 ...
 *
 * With `stackOverloads`, a second copy of every sampler-taking function is emitted
 * overloaded on `sampler2DArray`, sampling via `sampleStack(..., _stackT)` (both declared
 * by the caller's stack-helpers block). GLSL overload resolution picks by the declared
 * type of the texture uniform, so call sites are identical for singles and stacks.
 */
export const buildTriplanarDefsFragment = (
  ...
  vec4 triplanarTexture(${samplerType} map, vec3 pos, vec2 uvScale, vec3 normal, vec4 meanColor) {
    ...
    if (weights.x > 0.01) {
      outColor += ${sample('map', 'pos.yz * uvScale', 'meanColor')} * weights.x;
    }
    if (weights.y > 0.01) {
      outColor += ${sample('map', 'pos.zx * uvScale', 'meanColor')} * weights.y;
    }
    if (weights.z > 0.01) {
      outColor += ${sample('map', 'pos.xy * uvScale', 'meanColor')} * weights.z;
    }
    return outColor;
  }
  ...
  vec3 triplanarNormalMapPerturbation(${samplerType} map, vec3 pos, vec2 uvScale, vec3 normal, vec2 normalScale, vec4 meanColor) {
    ...
    vec2 tnormalX_xy = (${sample('map', 'pos.yz * uvScale', 'meanColor')}.xy * 2. - 1.) * normalScale;
    vec2 tnormalY_xy = (${sample('map', 'pos.zx * uvScale', 'meanColor')}.xy * 2. - 1.) * normalScale;
    vec2 tnormalZ_xy = (${sample('map', 'pos.xy * uvScale', 'meanColor')}.xy * 2. - 1.) * normalScale;
...
${stackOverloads ? samplerFns('sampler2DArray', (s, u) => `sampleStack(${s}, ${u}, _stackT)`) : ''}`;
};
