...
const encodeManifoldMesh = (manifold: Manifold) => {
...
export const apply_boolean = (
  ...
  const encoded = encodeManifoldMesh(outManifold);
...
export const simplify = (verts: Float32Array, indices: Uint32Array, tolerance: number) => {
  if (!ManifoldWasm) {
    throw new Error('Manifold Wasm not initialized');
  }
  ...
  const mesh = new Manifold(new Mesh({ numProp: 3, triVerts: indices, vertProperties: verts }));
  const simplified = mesh.simplify(tolerance);

  const encoded = encodeManifoldMesh(simplified);
...
export const convex_hull = (verts: Float32Array) => {
  ...
  const encoded = encodeManifoldMesh(manifold);
