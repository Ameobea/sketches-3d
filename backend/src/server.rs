...
use crate::{metrics::http_server, routes, settings::ServerSettings};
...
pub async fn start_server(settings: &ServerSettings) -> BootstrapResult<()> {
  let mut router = Router::new()
    .route(
      "/",
      axum::routing::get(instrument_handler("index", routes::index)),
    )
    .route(
      "/event/play_completed",
      axum::routing::post(instrument_handler("play_completed", routes::play_completed)),
    )
    .route(
      "/event/level_restarted",
      axum::routing::post(instrument_handler(
        "level_restarted",
        routes::level_restarted,
      )),
    )
    .route(
      "/event/portal_travel",
      axum::routing::post(instrument_handler("portal_travel", routes::portal_travel)),
    );
